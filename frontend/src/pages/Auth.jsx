...
OFFERS_PROMPT = (
    ...
    "Réponds UNIQUEMENT avec un JSON valide contenant une liste d'objets avec les clés: "
...
async def detecter_composant(image_base64: str) -> dict:
    ...
    try:
        ...
        logger.error(f"Erreur de parsing JSON IA : {e}")
        return {
            "name": "Composant inconnu",
            "category": "Erreur",
            "price_estimate": "0 €",
            "description": f"Erreur de lecture de l'IA: {raw[:100]}",
            "confidence": "Faible"
        }
...
async def _run_analysis(image_full_b64: str, user: dict) -> dict:
    ...
    parsed = await detecter_composant(image_full_b64)
...
async def get_offers(analysis_id: str, request: Request):
    ...
    try:
        chat = LlmChat(
            api_key=EMERGENT_LLM_KEY,
            session_id=f"offers-{uuid.uuid4()}",
            system_message=OFFERS_PROMPT,
        ).with_model("openai", "gpt-5.4")
        
        raw = await chat.send_message(UserMessage(text=f"Composant: {doc['name']} ({doc['category']}). Prix estimé: {doc['price_estimate']}. Propose les offres."))
        raw = raw if isinstance(raw, str) else str(raw)
