...
import { Shield, Lock, Mail, User, ArrowRight, Github } from "lucide-react";
...
const TOKYO_IMG = "https://images.unsplash.com/photo-1551641506-ee5bf4b45f1?crop-entropy=csp&cs=srgb&fm=jpg&ixid=M3w-14008zklxid=M3w-14008z%7CeyJhbGciOiJIUzI1NiIsImtpZCI6InN3aW5nLWw2bWF4IiwidmVyc2lvbiI6MS4wLCJ0eXBlIjoiY29tcG9uZW50LW1lZGl1bVwifQ&ixlib=rb-4.0.3&q=85";
  ...
  return (
    <div className="min-h-screen w-full flex bg-black text-white relative overflow-hidden">
      <div className="absolute inset-0 z-0 opacity-40">
        <img src={TOKYO_IMG} alt="Background" className="w-full h-full object-cover" />
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-between p-8 md:p-12">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-xl tracking-wider">ScanRescue</span>
        </div>

        <div className="max-w-md w-full mx-auto my-auto py-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight">Accéder à la plateforme</h1>
              <p className="text-sm text-muted-foreground mt-2">Connectez-vous ou créez un compte pour continuer.</p>
            </div>
            ...
            <Tabs defaultValue="login" className="w-full">
              ...
              <TabsContent value="login" className="space-y-4 pt-4">
                <form onSubmit={(e) => { e.preventDefault(); submit("login"); }} className="space-y-4">
                  ...
                  <Button data-testid="login-submit" type="submit" disabled={busy} className="w-full bg-neon-cyan text-black hover:bg-neon-cyan/80 font-bold h-11">
              ...
              <TabsContent value="register" className="space-y-4 pt-4">
                <form onSubmit={(e) => { e.preventDefault(); submit("register"); }} className="space-y-4">
                  ...
                  <Button data-testid="register-submit" type="submit" disabled={busy} className="w-full bg-neon-cyan text-black hover:bg-neon-cyan/80 font-bold h-11">
                    {busy ? "Création..." : "Créer un compte"} <ArrowRight className="ml-2 h-4 w-4" />
                  </Button>
                </form>
              </TabsContent>
            </Tabs>
            ...
            <Button data-testid="google-login" variant="outline" onClick={googleLogin} onMouseEnter={sfx.hover} className="w-full rounded-md border-white/20 bg-white text-black hover:bg-white/90 font-medium">
        ...
        <div className="text-xs text-muted-foreground text-center">
          ScanRescue &copy; 2026 - Tous droits réservés
        </div>
      </div>
    </div>
  );
...
function IconInput({ icon: Icon, testid, ...props }) {
  return (
    <div className="relative">
      ...
      <Input data-testid={testid} {...props} onChange={(e) => props.onChange(e)} className="pl-10 bg-black/50 border-white/10 text-white h-11 focus-visible:ring-neon-cyan" />
    </div>
  );
}
